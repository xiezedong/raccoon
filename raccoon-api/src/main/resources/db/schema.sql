...
CREATE TABLE IF NOT EXISTS discovery_tasks (
    ...
    processed_fields INT DEFAULT 0,
    current_table VARCHAR(100),
    current_column VARCHAR(100),
...
CREATE INDEX idx_discovery_tasks_created_at ON discovery_tasks(created_at DESC);

-- 添加注释
COMMENT ON TABLE discovery_tasks IS 'AI发现任务表';
COMMENT ON COLUMN discovery_tasks.status IS '状态: pending/running/completed/failed/cancelled';
